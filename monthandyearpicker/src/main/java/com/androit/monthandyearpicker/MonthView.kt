...
internal class MonthView @JvmOverloads constructor(
    ...
    // month
    private var _monthNames: Array<String>? = null
    ...
    private val _yearFormat: String? = null
    ...
    init {
        ...
        _monthHeaderSize = TypedValue.applyDimension(
        ...
        _monthSelectedCircleSize =
            if (resources.configuration.orientation == Configuration.ORIENTATION_PORTRAIT) {
                TypedValue.applyDimension(
                    TypedValue.COMPLEX_UNIT_DIP,
                    _monthSelectedCircleSize.toFloat(), displayMetrics
                ).toInt()
            } else {
                TypedValue.applyDimension(
                    TypedValue.COMPLEX_UNIT_DIP,
        ...
        _rowHeightKey = TypedValue.applyDimension(
            TypedValue.COMPLEX_UNIT_DIP,
            250f, displayMetrics
        ).toInt()
        _rowHeight = (_rowHeightKey - _monthHeaderSize) / MAX_NUM_ROWS

        _padding = TypedValue.applyDimension(
            TypedValue.COMPLEX_UNIT_DIP,
            16f, displayMetrics
        ).toInt()
    }
    ...
    private fun getMonthName(month: Int): String {
        if (_monthNames != null && _monthNames!!.size > 0) return _monthNames!![month]
    ...
    private fun drawDays(canvas: Canvas) {
        ...
        for (month in 0..11) {
            ...
            canvas.drawText(getMonthName(month), x.toFloat(), y.toFloat(), paint!!)
            j++
            if (j == _numDays) {
                j = 0
                y += _rowHeight
    ...
    fun setLocalFormats(locale: Locale?, monthFormat: String?) {
        _locale = locale
        _monthFormat = monthFormat
    }

    fun setMonthNamesArray(array: Array<String>?) {
        _monthNames = array
    }

    fun setMonthSelectedCircleSize(size: Int) {
        val displayMetrics = context.resources.displayMetrics
        _monthSelectedCircleSize = TypedValue.applyDimension(
            TypedValue.COMPLEX_UNIT_DIP,
            size.toFloat(), displayMetrics
        ).toInt()
    }
